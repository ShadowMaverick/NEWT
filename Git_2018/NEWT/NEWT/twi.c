...
void twi_init(void)
{
	...
	PORTD |= _BV(PORTD0);	// digitalWrite(SCL, 1);	DDRD |= _BV(DDD0);
	PORTD |= _BV(PORTD1);	// digitalWrite(SDA, 1);	DDRD |= _BV(DDD1);
...
void twi_disable(void)
{
	...
	PORTC &= ~_BV(PORTC4);	// digitalWrite(SDA, 0);
	PORTC &= ~_BV(PORTC5);	// digitalWrite(SCL, 0);
}
