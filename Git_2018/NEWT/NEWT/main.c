...
#define ENCODER_MID_POINT 7.9341f
...
char IMU_init();
void InitSSI();
char IMURead_eulerAngle(int);
char IMURead_eulerVelocities(int);
void ControlPID(int);
void ControlLQR(int);
...
int main(void)
{
	...
	// Starts USART (serial) communication to the raspberry pi (sets baud rate and enables Receiver/transmitter)
	USART_Init(BAUD);
	// Initialize communication with the IMU (sets rx tx buffers to 0)
	i2c_begin();
	i2c_setClock(IMU_CLOCK); //sets SCL clock frequency from IMU datasheet
	
	// Setup PWM for the precession motor (define motor enable, motor direction, pwm pins for velocity & precession 
	//and set enable high, direction cw and set pwm type and frequency using timer0, and duty cycles 0 for Precession, 
	//63% for velocity motor)
	InitPrecessionMotor();
	// Initialize timer interrupts (timer2 and 3 are initialized. timer2 is set to CTC mode - Clear Timer on Compare match
	// with 156
	InitClocks();
	// Initialize communication with the encoder (configure SPI elements)
	InitSSI();

	// Activate interrupts (starts all timers)
	sei();

	int iter = 0;
	while (1)
	{
		while (flag == 0); //code is stuck here for eternity unless flag changes to 1
		if (iter >= 100) {
			break;
		}
		iter++;
	...
	if (!IMU_init())
	{
		shutDown(); //precession shutdown, no velocity motor shutdown
		return 0;
	}

	flag = 0;

	// Wait for message from the pi before we continue (keeps asking you 32 times unless you press enter)
...
char IMURead_eulerAngle(int buffCount) // Gets new data from the IMU
{
	...
	//Serial.print("Starting Setup Transmission to IMU");
	i2c_beginTransmission(IMU_ID); //stores imu address and resets buffer
	//Serial.println("before Transmission to IMU");
	i2c_write_byte(0x42);//prepares sensor to receive TSS command
	i2c_write_byte(0x01);//Read filtered, tared orientation(Euler Angles) (gets stored in buffer)
	i2c_write_byte(0x43);//prepares sensor to send out data to master (reads buffer data stored)
	//Serial.println("After transmissions");
	int error = i2c_endTransmission(1);
	//Serial.print("Setup Transmission to IMU finished");
	...
	i2c_requestFrom(IMU_ID, 12, 0, 0, 1);//(initialization of imu)
	for (int i = 0; i < 3; i++)
	{
		temp_angle.angle = 0;
		for (int j = 3; j >= 0; j--)
...
char IMURead_eulerVelocities(int buffCount) // Gets new data from the IMU
{
	...
	//Serial.println("before Transmission to IMU");
	i2c_write_byte(0x42);
	i2c_write_byte(0x26); //read gyros rates
	i2c_write_byte(0x43);
	//Serial.println("After transmissions");
	int error = i2c_endTransmission(1);
	//Serial.print("Setup Transmission to IMU finished");
...
void ControlLQR(int buffCount) // Calculates and sends control values to the precession motor
{
	...
	control = min(abs(control), PREC_MAX_TORQUE); // saturating control input
	OCR0B = (uint8_t)(255 * (control/PREC_MAX_TORQUE));
}
...
void shutDown()
{
	...
	if (encfloat < 0.0f) PORTE &= ~_BV(PORTE3); // pin 5 direction: set low or high
	else PORTE |= _BV(PORTE3);
	OCR0B = 90;
