...
#define Prec1_PWM 6
#define Prec2_PWM 7
#define PREC_MAX_TORQUE 66.8f // from datasheet 
...
int ControlLQR(int buffCount) // Calculates and sends control values to the precession motor
{
  ...
  //analogWrite(Prec1_PWM, pwm_1); //! testing without precession motor
  //analogWrite(Prec2_PWM, pwm_1); //! testing without precession motor
...
void loop()
{
  ...
  while (Timer_Flag)
  {
    ...
    
    {
      ...
      Serial.println("****************IMU VELOCITY****************");
      Serial.println(IMUX_velocity[buffCount]);
      Serial.println("****************PRECESSION ANGLEsS****************");
      Serial.println(encfloat_1);
      Serial.println(encfloat_2);
      Serial.println("****************PRECESSION VELOCITIES****************");
