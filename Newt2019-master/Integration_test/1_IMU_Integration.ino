...
char IMU_init()
{
  ...
  int error = Wire.endTransmission();     //boolean
...
char IMURead_eulerAngle(int buffCount) // Gets new data from the IMU
{
  ...
  int error = Wire.endTransmission(); //! Review: can i2c_endTransmission be replaced by Wire.endTransmission //yes:)
...
char IMURead_eulerVelocities(int buffCount) // Gets new data from the IMU
{
  ...
  int error = Wire.endTransmission(); //! Review: can i2c_endTransmission be replaced by Wire.endTransmission
...
void setup(){
  ...
  /*Test Rig intialisation*/ 
    ...
    //initgyro(1); //! Not needed for IMU Testing
