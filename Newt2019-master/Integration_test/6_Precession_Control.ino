...
#define PREC_MAX_TORQUE 70 // from datasheet 
...
int ControlLQR(int buffCount) // Calculates and sends control values to the precession motor
{
  ...
  else
  {
    ...
    pwm_1 = round((4095 * (control_1/PREC_MAX_TORQUE))/5); /*changed 4095 to 255 since we go from 12 to 8 bit*/
    pwm_2 = round((4095 * (control_2/PREC_MAX_TORQUE))/5); /*changed 4095 to 255 since we go from 12 to 8 bit*/
...
void tc_setup() {
  ...
  TC0->TC_CHANNEL[2].TC_RC = 420000;  //<*********************  Frequency = (Mck/2)/TC_RC  Hz = 150 Hz  //! Changed freq. to 150 Hz
