...
#define MAXPRECESSION 45*M_PI/180 //! is math.pi 3.14..? //* 45 degrees is maximum precession angle, here converted to radians 
#define MINPRECESSION -45*M_PI/180 //* 45 degrees is minimum precession angle, here converted to radians

/** DEFINES FOR LQR Controller**/
#define SYS_FREQ 150            //Sampling frequency for the Control  
/*Gains for gyro nr 1: */
...
int ControlLQR(int buffCount) // Calculates and sends control values to the precession motor
{
  ...
  if (encfloat_1 > MAXPRECESSION || encfloat_1 < MINPRECESSION || encfloat_2 > MAXPRECESSION || encfloat_2 < MINPRECESSION){
      softstop();  
  }
  ...
  else
  {
    ...
    int pwm_1 = round(255 * (control_1/PREC_MAX_TORQUE)); /*changed 4095 to 255 since we go from 12 to 8 bit*/
    int pwm_2 = round(255 * (control_2/PREC_MAX_TORQUE)); /*changed 4095 to 255 since we go from 12 to 8 bit*/
...
void tc_setup() {
  ...
  TC0->TC_CHANNEL[2].TC_RC = 42000000/SYS_FREQ;  //<*********************  Frequency = (Mck/2)/TC_RC  Hz = 150 Hz  //! Changed freq. to 150 Hz
