...
int codeError;             //error code to specify type of error //! Review: Changed error status from buffer to a global var
...
//prec_velocity_1 = (encfloat_1 - oldEncfloat_1) / deltaClock; // ! Review: moved to encoder velocity fucntion (remove after revie) 
...
void loop()
{
  if (!IMU_init())
	...
	//TODO: Add a condition that waits for Serial input from RPi before starting everything!
	/*
	...
	*/
    ...
    
    {
      ...
      if (buffCount == BUFFER_SIZE){ // ! review: added condition to set flag when buffer limit is met // good!!
        flag = 1;
        break ;
      }
      if (digitalRead(RPi_PIN)) == HIGH){// ! review: added condition to set flag when RPi interupt is set //looks good
        flag = 2; 
        break;
      }
...
char IMURead_eulerVelocities(int buffCount) // Gets new data from the IMU
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		if (temp_angle_velocity.angle_velocity <= IMU_VEL_MAX) {
			euler_velocities[i] = -temp_angle_velocity.angle_velocity;	// Invert to match model //double check when mounted!!
		}
	}
